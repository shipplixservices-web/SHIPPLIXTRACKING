...
export default function App() {
  ...
  const activeTrackingQuery = searchParams.get("track") || "";
  ...
  const handleViewChange = (view: "customer" | "admin") => {
    if (view === "customer") {
      navigate("/");
    } else {
      navigate("/admin");
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-gray-900 flex flex-col selection:bg-[#FFD700] selection:text-[#032B73]">
      ...
      <main className="flex-grow max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
        <Routes>
          <Route path="/" element={<CustomerPortal initialTrackingQuery={activeTrackingQuery} />} />
          <Route path="/admin" element={<AdminPanel onTrackingRequest={handleAdminViewShipment} />} />
